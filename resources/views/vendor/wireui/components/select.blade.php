<div {{ $attributes->only(['class', 'wire:key'])->class('relative w-full') }} x-data="wireui_select({
    @if ($attributes->wire('model')->value()) wireModel: @entangleable($attributes->wire('model')), @endif
})"
    x-props="{
    ...
    <div class="relative">
        @if ($label)
            <x-dynamic-component :component="WireUi::component('label')" class="mb-1" :label="$label" :has-error="$name && $errors->has($name)" :disabled="$disabled"
                x-on:click="toggle" :wire:key="'select.label.' . $name" />
        @endif
        ...
        <x-dynamic-component :component="WireUi::component('input')"
            ...
            x-ref="input" x-on:click="toggle" x-on:focus="open" x-on:blur.debounce.750ms="closeIfNotFocused"
            x-on:keydown.enter.stop.prevent="toggle" x-on:keydown.space.stop.prevent="toggle"
            x-on:keydown.arrow-down.prevent="$event.shiftKey || getNextFocusable().focus()"
            x-on:keydown.arrow-up.prevent="getPrevFocusable().focus()" x-bind:placeholder="getPlaceholder"
            x-bind:value="getSelectedValue" inputmode="none" readonly autocomplete="false" :name="$name"
                ...
                <div
                    ...
                    <template x-if="!config.multiselect">
                        <div @class([
                            ...
                            <span class="text-sm truncate text-secondary-700 dark:text-secondary-400"
                                    ...
                                    <template x-for="(option, index) in selectedOptions" :key="`selected.${index}`">
                                        <span
                                            ...
                                            <button
                                                ...
                                                <x-dynamic-component :component="WireUi::component('icon')" class="w-3 h-3"
                                                    name="x" />
                                            </button>
                                        </span>
                                    </template>
    ...
    <x-wireui::parts.popover :margin="(bool) $label" class="sm:max-w-xs">
        ...
        <div class="overflow-y-auto select-none max-h-64 sm:max-h-60 overscroll-contain soft-scrollbar" tabindex="-1"
            x-ref="optionsContainer" name="wireui.select.options.{{ $name }}"
