    ...
    <div class="overflow-x-auto">
        <table
            ...
            <tbody>
                ...
                @forelse($subelemenData as $subelemen)
                    <tr wire:key="{{ $subelemen->id }}"
                        class="text-center bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <th scope="row"
                            class="px-2 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {{ $subelemenData->firstItem() + $loop->index }}
                        </th>
                        <td scope="col" class="px-4 py-4 max-w-[32rem]">
                            {{ Str::of($subelemen->dimensiDeskripsi)->words('25', ' ...') }}
                        </td>
                        <td scope="col" class="px-4 py-4 max-w-[32rem]">
                            {{ Str::of($subelemen->elemenDeskripsi)->words('25', ' ...') }}
                        </td>
                        <td scope="col" class="px-4 py-4 max-w-[32rem]">
                        ...
                        <td class="px-4 py-4">
                            <x-button.circle green icon="pencil-alt"
                                href="{{ route('subelemenEdit', ['subelemen' => $subelemen->id]) }}" wire:navigate />
                            <x-button.circle negative icon="trash"
                                x-on:click="$dispatch('set-subelemen', {{ $subelemen->id }}); $openModal('deleteModal');" />
                        </td>
                    </tr>
                @empty
                    <tr
