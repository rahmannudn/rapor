            ...
            @forelse($mapelData as $data)
                <tr wire:key="{{ $data->id }}"
                    ...
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        {{ $mapelData->firstItem() + $loop->index }}
                    </th>
                    <td class="px-4 py-4">
                        {{ $data->nama_mapel }}
                    </td>
                    <td class="px-4 py-4">
                        <x-button.circle green icon="pencil-alt" href="{{ route('mapelEdit', ['mapel' => $data]) }}"
                            wire:navigate />
                        <x-button.circle negative icon="trash"
                            x-on:click="$dispatch('set-mapel', {{ $data->id }}); $openModal('deleteModal');" />
                    </td>
                </tr>
            @empty
                <tr
