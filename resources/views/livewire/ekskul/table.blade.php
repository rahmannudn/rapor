        ...
        <tbody>
            @forelse($ekskulData as $data)
                <tr wire:key="{{ $data->id }}"
                    ...
                    <td class="px-4 py-4">
                        <x-button.circle green icon="pencil-alt" href="{{ route('ekskulEdit', ['ekskul' => $data]) }}"
                            wire:navigate />
                        <x-button.circle negative icon="trash"
                            x-on:click="$dispatch('set-ekskul', {{ $data->id }}); $openModal('deleteModal');" />
                    </td>
                </tr>
            @empty
                <tr
